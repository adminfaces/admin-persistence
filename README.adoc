...
Extend `BaseEntity` (or implement `PersistenceEntity` interface) on your JPA entities:
...
Extend CrudMB and provide your service to your JSF managed beans to enable CRUD support out of the box:
...
You can change this behavior by overriding  keepFiltersInSession method on you Bean:
